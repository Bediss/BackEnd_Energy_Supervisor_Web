...
def cloneSynopticV1(request):
    if request.method == "POST" or request.method == "PUT":
        ...
        for synoptic_index in range(len(S_IDs)):
            __synoptic = next(
                (item for item in response if item["Report_Code"] == S_IDs[synoptic_index]), None)

            synoptic = copy.deepcopy(__synoptic) if type(
                __synoptic) is dict else None
            ...
            if synoptic.get("Body") is None or synoptic.get("Selected_Global") is None:
                output.append(None)
                continue
            synopticOutput = cloneSynopticDev_V1(
                synoptic, synopticData, tasker, save)
        ...
        if save is True:
            ...
            clonedReportsCount = len(
                list(filter(clonedReportsCountFilter, output)))
            ...
            for report in output:
                ...
                report["Report_Code"] = maxCodes.pop()
                report["DBAction"] = 2
...
def cloneSynopticDev_V1(synoptic, synopticData, tasker, save):
                ...
                if memberType == m:
                    ...
                    if affix.lower() == "params":
                        ...
                        for elem in memberData:
                            __from = globalDataElement[elem] if elem in globalDataElement else None
                            __to = memberData[elem] if elem in memberData else ""

                            if __from is not None and __to is not None:
                                workLoadClone.append(
                                    {"from": {"params": __from}, "to": {"params": __to}})
    ...
    synopticBody = renameElements_dummy_s_V1(workLoadClone, synopticBody)
    ...
    if save is True:
        ...
        Selected_Global = globalDataHandler_s_V1(
...
def globalDataHandler_s_V1(synopticBody, Selected_Global, _type, sgf=None):
...
def renameElements_dummy_s_V1(task: list, report: dict = dict()):
    ...
    if request.method == "POST" or request.method == "PUT":
        ...
        for synoptic_index in range(len(S_IDs)):
            ...
            if synoptic.get("Body") is None or synoptic.get("Selected_Global") is None:
                output.append(None)
                continue
            synopticOutput = cloneSynopticDev_V2(
                synoptic, synopticData, tasker, save, superClone)
            tasker.closeConnection()
...
def cloneSynopticDev_V2(synoptic, synopticData, tasker, save, superClone=False):
    ...
    Report_TableauName = synoptic["Report_TableauName"].strip(
    ) if "Report_TableauName" in synoptic else ""
    reportName = "{}{}{}{}".format(
        Report_TableauName, clTag, mlTag, tlTag).strip()
