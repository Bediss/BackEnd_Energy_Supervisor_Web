...
def cloneReportV3(R_IDs: list, data: list, tasker, save=False, superClone=False):
    ...
    for report_index in range(len(R_IDs)):
        ...
        reportWithReplacedElements = renameElements_dummyV3(
        ...
        if save is True:
            ...
            Selected_GlobalWithReplacedElements = globalDataHandlerV3(
                workLoadCopy, Selected_Global, "copy")

            Selected_GlobalWithReplacedElements = globalDataHandlerV3(workLoadClone, Selected_GlobalWithReplacedElements, "clone", {
                "ml": ml, "cl": cl, "tl": tl})
...
def globalDataHandlerV3(workLoad, _globalData, _type, dim=None):
    ...
    if _type == "clone":
        ...
        for i in range(len(globalData)):
            ...
            for wl in workLoad:
                ...
                firstValue = cleanupV3(_from[firstKey], mct)

                secondkey = list(_from.keys())[1]
                secondValue = cleanupV3(_from[secondkey], mct)
...
def renameElements_dummyV3(task: list, report: dict = dict()):
