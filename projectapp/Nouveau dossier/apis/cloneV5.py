...
def cloneReport_api(request):
    if request.method == 'POST' or request.method == 'PUT':
        ...
        clonedReports = cloneReport(R_IDs=R_IDs, data=data, tasker=tasker, save=save, superClone=superClone)
        ...
        if save is True:
            ...
            for ii in range(len(clonedReports)):
                ...
                report["Selected_Global"] = json.loads(json.dumps(report["Selected_Global"]).replace("'", "''")) if "Selected_Global" in report else dict()
        ...
        for rep in clonedReports:
            clonedReportsOut.append(rep["Body"] if rep is not None and "Body" in rep else None)
...
def cloneReport(R_IDs: list, data: list, tasker, save=False, superClone=False):
    ...
    for report_index in range(len(R_IDs)):
        ...
        Selected_Global = report["Selected_Global"] if type(report) is dict and "Selected_Global" in report else None
        ...
        tlIot = {"type": "tlIot", "reportData": tlIot} if type(tlIot) is dict else None
        ...
        tl = next(item for item in Selected_Global if item["Dim"].lower() == "tl")
 
        tlVar = tl["Dim_type"].lower() == "var"

        tlClusterLabel = tl["Dim_label"].get("cluster_name","") if "Dim_label" in tl else ""
        tlIotLabel = tl["Dim_label"].get("iot_name","") if "Dim_label" in tl else ""
        ...
        Report_TableauName = report["Report_TableauName"].strip() if "Report_TableauName" in report else ""

        iotTag=    tlIot["reportData"]["tag"]     if tlIot is not None and type(tlIot["reportData"].get("tag",None)) is str else None
        ...
        if iotTag is None and tlIot is not None:
            ...
            iotTag=json.loads(iotTag).get(tlIot["reportData"]["id"],dict()).get("name")
        ...
        if clusterTag is None and tlCluster is not None:
            if tlCluster["reportData"]["id"] !='now':
                ...
                clusterTag=json.loads(clusterTag).get(tlCluster["reportData"]["id"],dict()).get("name")
                if type(clusterTag) is not str:
                    return 400
                tlCluster["reportData"]["tag"]=clusterTag
            else:
                tlCluster["reportData"]["tag"]=''
        # iotTag=iotTag if iotTag is not None else ""
        # clusterTag=clusterTag if clusterTag is not None else ""
        # reportName =title if title is not None else "{} {} {} {}".format(Report_TableauName.strip(), clTag.strip(), mlTag.strip(),iotTag.strip())
        reportName =title if title is not None else "{} {} {} {}".format(Report_TableauName.strip(), clTag.strip(), mlTag.strip(),iotTag.strip() if iotTag is not None else clusterTag.strip() if clusterTag is not None else "")
                        ...
                        if globalDataElement["clone"] is True:
                            ...
                            if affix.lower() == "tliot":
                                ...
                                for i in range(len(body["objects"])):
                                    ...
                                    if query == "iotinner":
                                        isTlX = obj["MasterObj_Data_selection"]["x"].lower() == "tl"
                                        isTlPage = obj["MasterObj_Data_selection"]["page"]["page"].lower() == "tl"
                                ...
                                for i in range(len(body["objects"])):
                                    ...
                                    if query == "cluster":
                                        ...
                                        tag = tl["tag"]
                                        if id=="now":
                                            obj["MasterObj_Data_Query"].pop("tl",None)
                                        else:
                                            obj["MasterObj_Data_Query"].update({"tl": id})
                                ...
                                for member in memberData:
                                    ___to = member
                                    ___to[affix] = cleanup( member[affix], memberType)
                                    ___from = next((item for item in globalDataElement["members"] if item[affix] == ___to[affix]), None)
                                    if ___from is not None and ___to is not None:
                                        workLoadClone.append({"from": ___from, "to": ___to})
                            ...
                            if l["lCopy"]:
                                for sri in range(len(report["Body"]["objects"])):
                                    ...
                                    isPage = sr["MasterObj_Data_selection"]["page"]["type"].lower() != "a"
                                ...
                                for i in range(max):
                                    ...
                                    ___from = globalDataElement["members"][i] if len(globalDataElement["members"]) > i else None
                                    ___to = memberData[i] if len(memberData) > i else None

                                    if ___from is not None and ___to is not None:
                                        if type(___from) is dict and type(___to) is dict:
                                            if None not in list(___from.values()) and None not in list(___to.values()):
                                                workLoadCopy.append({"from": ___from, "to": ___to})

        # print(json.dumps(workLoadClone,indent=4))
        reportWithReplacedElements = renameElements_dummy(workLoadClone, report)
        reportWithReplacedElements = renameElements_dummy(workLoadCopy, reportWithReplacedElements)
...
def globalDataHandler(workLoad, _globalData, _type, dim=None):
    ...
    if _type == "clone":
        ...
        for i in range(len(globalData)):
            ...
            clone=next((True for e in globalData if e["Dim"].lower()=="tl"),False)
        ...
        if len(elem) == 2:
            ...
            if _from is not None and _to is not None:
                ...
                if _fromNameValue != _toName and _fromCodeKey != "SQL" and _fromCodeKey != "SQLc":
                    _report = _report.replace('"{}"'.format(_fromNameValue), '"{}"'.format(_toName))
