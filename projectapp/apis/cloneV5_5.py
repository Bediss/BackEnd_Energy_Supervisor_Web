...
def cloneReport(R_IDs: list, data: list, tasker, save=False, superClone=False):
    ...
    for report_index in range(len(R_IDs)):
        ...
        tlCluster = {"type": "tlCluster", "reportData": tlCluster} if type(
            tlCluster) is dict else None
        ...
        tlClusterLabel = tlSG["Dim_label"].get(
            "cluster_name", "") if "Dim_label" in tlSG else ""
        tlIotLabel = tlSG["Dim_label"].get(
            "iot_name", "") if "Dim_label" in tlSG else ""
        tlClone = tlSG["Dim_Clone"] is True
        tlBucker = tlSG["Dim_Bucket"] is True if "Dim_Bucket" in tlSG else False
        tlClusterMembers = tlSG["Dim_Member"]["clusterMembers"] if "clusterMembers" in tlSG["Dim_Member"] else None
        tlIotMembers = tlSG["Dim_Member"]["iotinnerMembers"] if "iotinnerMembers" in tlSG["Dim_Member"] else None
        ...
        if iotTag is None and tlIot is not None:
            for iotTag_index in  tlIot["reportData"]["ids"]:
                id=tlIot["reportData"]["ids"][iotTag_index]
                _tl=_getTl2(id, "Tl_User")
                _iotTag = json.loads(_tl).get(id, dict()).get("name",None)
                if _iotTag is not None and id not in iotTags:
                    iotTags[iotTag_index]=_iotTag
            if len(iotTags)<1:
                return 400
            tlIot["reportData"]["tag"] = iotTags
        ...
        if save is True:
            ...
            Selected_GlobalWithReplacedElements = globalDataHandler(
