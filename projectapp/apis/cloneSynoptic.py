...
# api
def cloneSynopticV2(request,newBody=None):
...
def cloneSynopticDev_V2(synoptic, synopticData, tasker, save, superClone=False):
    ...
    #     # tl = list([synopticData["tl"]["id"]])
    #     # tlTag = synopticData["tl"]["tag"]
    #     tl = list([synopticData["tlCluster"]["id"]])
    #     tlTag = synopticData.get("tlCluster",dict()).get("tag","")
    if "tlCluster" in synopticData:
        tl = list([synopticData["tlCluster"]["id"]])
        tlTag = synopticData.get("tlCluster",dict()).get("tag","")
    ...
    tl = {"type": "tl", "synopticData": tl, "tag": tlTag} if type(tl) is list else None
        ...
        for l in [ml, cl, tl, params]:
            ...
            if l is not None:
                ...
                if memberType == m:
                    ...
                    affix = "m_name" if memberType == "ml" else "Le_Compteur" if memberType == "cl" else "tl" if memberType == "tl" else "params"
                    
                    if affix.lower() == "params":
                        for elem in memberData:
                            synopticBody["object"][elem] = memberData[elem]
                    elif affix.lower() == "tl":
                        synopticBody["object"]["MasterObj_Data_Query"]["tl"] = memberData.pop()
                    else:
