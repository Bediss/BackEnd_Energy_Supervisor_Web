...
class Back_DB_bridge(object):
    ...
    def display(self, task):
        ...
        if Header_list != "*":
            ...
            if (len(QUERY_VALUE[MAX_Fields - 1].split(",")) > 1):
                for j in value_number:
                    value_field = value_field + "'{}'" + \
                        ",".format(list_value_field[j])

                value_field = value_field + \
                    "'{}'".format(list_value_field[value_number - 1])

                where_query = where_query + '"{}"' + \
                    " in ( {} ) ".format(
                        QUERY_HEADER[MAX_Fields - 1], value_field)
            # end if list value of field supp 1
            else:
                if QUERY_VALUE[MAX_Fields - 1] == '%':
                    where_query = where_query + '"{}" like ' + \
                        "'{}'".format(
                            QUERY_HEADER[MAX_Fields - 1], QUERY_VALUE[MAX_Fields - 1])
                else:
                    where_query = where_query + '"' + \
    ...
    def update(self, task):
        ...
        try:
            ...
            # self.db.addTransaction("truncate_table", [temp_table])
            # print("create_table_as_select :::: ",temp_table,"<-------->", modify_table)
            # print("insert_from_json :::: ",temp_table,"<-------->", data)
            # print("delete_proc :::: ",tableName,"<-------->", temp_table)
            # print("add_proc :::: ",tableName,"<-------->", temp_table)
            # print("update_proc :::: ",tableName,"<-------->", temp_table)
            print("----------------------------")
            self.db.addTransaction("create_table_as_select", [temp_table, modify_table])
            self.db.addTransaction("insert_from_json", [temp_table, data])
            self.db.addTransaction("delete_proc", [tableName, temp_table])
            self.db.addTransaction("add_proc", [tableName, temp_table])
            self.db.addTransaction("update_proc", [tableName, temp_table])
            self.db.addTransaction("drop_table", [temp_table])
    ...
    def cluster(self, task):
        ...
        # input params
        ML_json = json.dumps(task.get("ml",dict())).strip()
        CL_json = json.dumps(task.get("cl",dict())).strip()
        CL_ID   = task.get("tl",None)
        ...
        self.db.execProc("create_table_as_select", [ML_table_name, 'ml_temp'])
        ...
        self.db.execProc("create_table_as_select", [CL_table_name, "cl_temp"])
        ...
        return self.__cluster_work(self.db, type_result, output_type, ML_table_name, CL_table_name, table_name_pbi_normalised, table_name_pbi_crosstab,CL_ID)

    def __cluster_work(self, db, type_result, output_type, ML_table_name, CL_table_name, table_name_pbi_normalised, table_name_pbi_crosstab,CL_ID):
        output="NULL"
